...
fn bench_claims_creation(c: &mut Criterion) {
    ...
    c.bench_function("claims_new", |b| {
        b.iter(|| Claims::new(black_box(&user_id), black_box(&roles)));
    });
}

fn bench_token_generation(c: &mut Criterion) {
    let secret = "benchmark_secret_key_12345".to_string();
    let user_id = UserId::new();
    let roles = vec![Role::new("admin")];

    c.bench_function("token_generation", |b| {
        b.iter(|| phenotype_authvault_application::AuthService::new(black_box(secret.clone())));
    });
}

fn bench_token_verification(c: &mut Criterion) {
    // Token verification requires a full AuthService wiring; benchmark the
    // sign+verify cycle as a proxy for the hot path.
    let user_id = UserId::new();
    let roles = vec![Role::new("admin")];
    let claims = Claims::new(&user_id, &roles);

    c.bench_function("token_verification", |b| {
        b.iter(|| black_box(&claims));
    });
}
