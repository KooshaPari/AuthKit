...
use phenotype_authvault_domain::ports::{SessionStorage, UserStorage};
use phenotype_authvault_domain::{Session, SessionId, User, UserId};
    ...
    async fn delete_expired(&self) -> Result<usize, String> {
        ...
        for (id, session) in sessions.iter() {
            if session.expires_at < now
                || session.state != phenotype_authvault_domain::session::SessionState::Active
            {
...
mod tests {
    use super::*;
    use phenotype_authvault_domain::User;
    ...
    async fn test_session_storage() {
        ...
        let session = phenotype_authvault_domain::Session::new("user-123");
